class ParticipantScoresController < ApplicationController
  ...
  def index
    ...
    @trip_participant = TripParticipant.find(params[:trip_participants_id])
  ...
  def create_remianing_scoring_records(trip_participant)
    @trip = trip_participant.trip
    trip_participants = TripParticipants.where(trip: @trip)
    ...
    potentially_destinations.each do |pd|
      next unless ParticipantScore.find_by(potential_destination: pd).null?
